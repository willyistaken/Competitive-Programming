...
int main() {
    ...
    while (t--) {
        ...
        short timenow = 0;
        while (!master.emtpy() || maxcount<=count_target) {
            ...
            if((colormap[ master.front().x+dx[i]][master.front().y+dy[i]] | master.front().c ) !=colormap[(short) master.front().x+dx[i]][(short) master.front().y+dy[i]]){
                ...
                if (colormap[master.front().x+dx[i]][master.front().y+dy[i]]==target) ++count_target;
                 master.push({(short) (master.front().x)+(dx[i]),(master.front().y)+(dy[i]),colormap[master.front().x+dx[i]][master.front().y+dy[i]],(uint16_t) master.front().t+1});
                         }
